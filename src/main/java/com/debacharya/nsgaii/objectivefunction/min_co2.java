                ...
                if(!m.contains(i)){//如果i不是枢纽节点
                    ...
                    if(!m.contains(j)){//如果j不是枢纽节点
                        ...
                        ans+=((all_transport+airplane1-1)/airplane1)*
                                (city_distance[i][target1]+city_distance[target2][j])*2*co2_1+up_down1;//从j到i和从i到j，路程需要算两次
                        if(target1!=target2){//如果不是同一个枢纽机场和枢纽机场,还存在一段枢纽机场->枢纽机场的运输
                            ans+=((all_transport+airplane2-1)/airplane2)*
                                    city_distance[target1][target2]*2*co2_2+up_down2;
                        }
                    }
                    else{//如果j是枢纽节点
                        ans+=((all_transport+airplane1-1)/airplane1)*
                                city_distance[i][target1]*2*co2_1+up_down1;
                        if(target1!=j)//如果j部是和i直连的
                            ans+=((all_transport+airplane2-1)/airplane2)*
                                    city_distance[target1][j]*2*co2_2+up_down2;
                    }
                }
                else {//如果i是枢纽节点
                    if(!m.contains(j)){//如果j不是枢纽节点
                        ...
                        int target1=Integer.parseInt(no_hinge_citys[no_hign_index1]);
                        ans+=((all_transport+airplane1-1)/airplane1)*
                                (city_distance[j][target1])*2*co2_1+up_down1;
                        if(target1!=j){
                            ans+=((all_transport+airplane2-1)/airplane2)*
                                    city_distance[target1][i]*2*co2_1+up_down2;
                        }
                    }
                    else{//j是枢纽节点
                        ans+=((all_transport+airplane2-1)/airplane2)*
                                city_distance[j][i]*2*co2_2+up_down2;
                    }
                }
            }
        }
